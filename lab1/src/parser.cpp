...
std::map<int, std::string> Parser::getRuleIndexes() {
...
void Parser::addRule(char from, std::string to) {
...
std::vector<int> Parser::analyze(std::string str) {
        ...
        switch (state) {
            ...
            case Goto: 
            {
                ...
                if(grammar.rules[l1.top().symbol].size() > (l1.top().index + 1)) { // step 6a
                    Element elem = l1.top();
                    popNonTerminal(l2, elem);
                    l1.pop();

                    elem.index++;
                    l1.push(elem);
                    pushNonTerminal(l2, elem);
...
void Parser::popNonTerminal(std::stack<char> &l2, Element elem) {
...
void Parser::pushNonTerminal(std::stack<char> &l2, Element elem) {
    std::string rule = grammar.rules[elem.symbol][elem.index];
